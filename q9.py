...
# Configuration - Résolution augmentée
NB_LIGNES = 60
NB_COLONNES = 80
TAILLE_CASE = 10
...
class Monde2D:
    def __init__(self, root):
        self.root = root
        self.root.title("Monde 2D avec Règles de Conway")
    ...
    def setup_interface(self):
        ...
        regles = [
            ...
            ("Règles de Conway", self.var_conway),
            ("Auto (0.5s)", self.var_auto),
        ]
        for texte, var in regles:
            tk.Checkbutton(self.frame_gauche, text=texte, variable=var, bg="#f0f0f0", command=self.toggle_auto if texte == "Auto (0.5s)" else None).pack(anchor=tk.W, padx=10)
    ...
    def auto_step(self):
        if self.is_auto_running and self.var_auto.get():
            self.next_step()
            self.root.after(500, self.auto_step)  # 500ms = 0.5s
    ...
    def count_neighbors(self, x, y):
        """Compte les voisins vivants autour d'une cellule pour les règles de Conway"""
        count = 0
        for dy in [-1, 0, 1]:
            for dx in [-1, 0, 1]:
                if dx == 0 and dy == 0:
                    continue
                nx, ny = x + dx, y + dy
                if 0 <= nx < NB_COLONNES and 0 <= ny < NB_LIGNES:
                    count += self.monde[ny][nx]
        return count

    def apply_conway_rules(self):
        """Applique les 4 règles de Conway"""
        new_monde = [[0 for _ in range(NB_COLONNES)] for _ in range(NB_LIGNES)]
            ...
            for x in range(NB_COLONNES):
                ...
                if cell == 1:  # Cellule vivante
                    if neighbors == 2 or neighbors == 3:
                        new_monde[y][x] = 1  # Survit
                    else:
                        new_monde[y][x] = 0  # Meurt
                else:  # Cellule morte
                    if neighbors == 3:
                        new_monde[y][x] = 1  # Naît
                    else:
                        new_monde[y][x] = 0  # Reste morte
        
        self.monde = new_monde
        for y in range(NB_LIGNES):
            for x in range(NB_COLONNES):
                if self.monde[y][x] == 1 and self.var_esperance.get():
                    self.vies[y][x] = random.randint(5, 8)
                elif self.monde[y][x] == 0:
                    self.vies[y][x] = 0
